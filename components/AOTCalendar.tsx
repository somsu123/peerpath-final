...
const AOTCalendar: React.FC<AOTCalendarProps> = ({ currentUser }) => {
    ...
    const handleCreateEvent = () => {
        ...
        if (!isRepresentative) {
            alert('Only representative students can create events. Enable representative status in your profile settings.');
            return;
        }
    ...
    const handleDeleteEvent = (eventId: string) => {
        if (!isRepresentative) {
            alert('Only representative students can delete events.');
            return;
        }
    ...
    return (
        <div className="max-w-7xl mx-auto">
            ...
            <div className="mb-12">
                ...
                {!isRepresentative && currentUser && (
                    <div className="mt-4 bg-amber-50 border border-amber-200 rounded-2xl p-4 flex items-start gap-3">
                        <AlertCircle className="w-5 h-5 text-amber-600 flex-shrink-0 mt-0.5" />
                        <div>
                            <p className="text-sm font-bold text-amber-800">Representative Access Required</p>
                            <p className="text-xs font-semibold text-amber-600 mt-1">
                                Enable representative status in your profile settings to create and manage events.
                            </p>
                        </div>
                    </div>
                )}
